...
main() {
    ...
    # Thus this workaround removes ".computeResources.requests" from the "build" step and updates the build Pipeline that is used in CI to reference the updated Task
    yq e -i "del(.spec.steps[] | select(.name == \"build\").computeResources.requests)" $tmp_build_task_path
